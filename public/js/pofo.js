(($)=>{
  ...
  class pofo {
      ...
      section5(){
        ...
        let piece = []; //초당 작은 조각단위의 길이
        let second = 3;
        let perSize = []; //퍼센트의 해당하는 길이를 저장할 배열변수
        let percent = [.9, .75, .9, 0.62];
        let sum = []; // 조각이 누적되는 배열변수 : ** 누적변수는 반드시 초기값이 0이 필요하다.
        let sumId = []; // 타이머 정지 변수
            ...
            function svgAnifn(){
              $.each(svgCicle, function(idx, obj){
                ...
                //3. 각 요소(원형)의 길이를 백분율을 구현하기위해 작은 조각단위의 길이를 구한다.
                //   1초동안의 원의 작은 조각단위(piece 피이스  chip 칩)의 길이
                piece[idx] = svgArr[idx] / second / 100;

                //4. 퍼센트에 길이를 구한다.
                perSize[idx] = svgArr[idx] * percent[idx];

                //5. 함수이용 조각단위를 누적하는 함수
                function sumfn(){
                  sum[idx] += piece[idx]; //조각단위 누적 함
                  if(sum[idx]>perSize[idx]){
                    clearInterval(setId[idx]);
                  }
                  else { //원형 svg 애니메이션 구현
                ...
                //6. 타이머
                sumId[idx] = setInterval(sumfn, 10);
